...
=== "r1.1"
    ...
    Pressionar o botão BOOT, reiniciar a alimentação da placa (ou pressionar o botão RESET) e soltar BOOT.
    O microcontrolador será iniciado no modo DFU.
...
Se não houver permissões, podem ocorrer erros durante a gravação; para obter acesso, execute o comando:
```
...
- Verificar: 
    ...
    ```iHeater pronto para funcionar com Klipper```
